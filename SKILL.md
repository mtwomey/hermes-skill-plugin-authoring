...
A plugin repo has exactly 6 files at the root (plus an optional `scripts/` dir):

```
hermes-skill-<plugin-name>/
...
├── setup.py           ← Install/uninstall/credentials CLI
...
- **Repo dir:** `~/Git_Repos/hermes-skill-<plugin-name>/`
...
```bash
mkdir ~/Git_Repos/hermes-skill-<plugin-name>
...
- [ ] Repo created at `~/Git_Repos/hermes-skill-<plugin-name>/` with `git init`
...
- [ ] `__init__.py` — `_REGISTRY` complete; `ctx.register_tool(name=..., toolset=..., ...)` — both `name=` and `toolset=` present; `ctx.register_skill(...)` present
- [ ] `SKILL.md` — all sections filled in; `## Common Patterns` has real rows
- [ ] `setup.py` — tokens replaced; `CRED_PROMPTS` correct
